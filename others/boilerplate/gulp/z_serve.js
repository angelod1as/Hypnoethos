...
gulp.task('browser', gulp.parallel(() => {
	...
	gulp.watch(path.join(pkg.src.others, '**/*'), gulp.parallel('others'));
	...
	browserSync({
		...
		files: [pkg.build.css + '**/*.css', pkg.build.images + '**/*', pkg.build.script + '**/*.js', pkg.build.others + '**/*', pkg.paths.build + '**/*.html']
	});
